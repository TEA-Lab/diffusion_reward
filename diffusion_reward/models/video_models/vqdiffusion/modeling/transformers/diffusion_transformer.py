...
class DiffusionTransformer(nn.Module):
    ...
    def log_sample_categorical(self, logits, noise=True, noise_scale=1):           # use gumbel to sample onehot vector from log probability
        if noise:
            uniform = torch.rand_like(logits)
            gumbel_noise = -torch.log(-torch.log(uniform + 1e-30) + 1e-30) * noise_scale
    ...
    def q_sample(self, log_x_start, t, noise=True):                 # diffusion step, q(xt|x0) and sample xt
        log_EV_qxt_x0 = self.q_pred(log_x_start, t)

        log_sample = self.log_sample_categorical(log_EV_qxt_x0, noise)
    ...
    def sample_fast(
        ...
        output = {'content_token': content_token}
        output['log_probs'] = model_log_prob
        if return_logits:
            output['logits'] = torch.exp(log_z)
        return output
